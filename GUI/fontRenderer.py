...
class FONTRENDERER:
    ...
    def _currentStringData(self, String):
        ...
        for i in range(self._stringLenght):
            ...
            if self._stringList[i] != String[i] or self._fullUpdate is True:
                #print("Update string (fonterenderer)", String)
                texPos = np.array([self._FontLoader.xPos[Index], -self._FontLoader.yPos[Index] - self._FontLoader.height[Index]])/self._FontLoader.imageSize
                texScale = np.array([self._FontLoader.widht[Index], self._FontLoader.height[Index]])/self._FontLoader.imageSize
                pos = np.array([self._FontLoader.xOffset[Index]*2.0 + xAdvance*2.0 + self._FontLoader.widht[Index], -self._FontLoader.yOffset[Index]*2.0 - self._FontLoader.height[Index]])
                ...
                self._maxStringHeight = max(self._maxStringHeight, abs(-self._FontLoader.yOffset[Index]*2.0 - self._FontLoader.height[Index]*2.0))
                ...
                self._updateStringData(texPos, i, self._textPos_VBO)
                self._updateStringData(texScale, i, self._textScale_VBO)
                self._updateStringData(pos, i, self._pos_VBO)
                self._updateStringData(scale, i, self._scale_VBO)
    ...
    def _initiateQuad(self):
        ...
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, QuadVert.itemsize * 5, ctypes.c_void_p(0))
        glEnableVertexAttribArray(1)
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, QuadVert.itemsize * 5, ctypes.c_void_p(12))
    ...
    def _updateStringData(self, Data, Index, Buffer):
        NewData = np.array(Data, np.float32).flatten()
        glBindBuffer(GL_ARRAY_BUFFER, Buffer)
        glBufferSubData(GL_ARRAY_BUFFER, int(Index * 4 * 2), int(4 * 2), NewData)
